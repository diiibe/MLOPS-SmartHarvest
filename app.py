...
def reuse_project():
    ...
    try:
        ...
        analysis_progress[project_name_safe] = {'status': 'Regenerating outputs...', 'percent': 50}
                ...
                for item in meta_list:
                    if 'source' in item:
                        if item['source'] == 'ROI Stats':
                            stats.append({'label': "Area (ha)", 'value': f"{item.get('area_ha', 0):.2f}"})
                            stats.append({'label': "Analysis Window", 'value': item.get('analysis_range', 'N/A')})
                        elif item['source'] == 'SRTM':
                            topo_stat = {'label': "Topography", 'value': "Static (SRTM)"}
                        elif 'image_count' in item:
                            image_stats.append({'label': f"{item['source']} Images",
    ...
    if not stats and os.path.exists(csv_path):
        try:
            ...
            if 'date' in df.columns:
                ...
                stats.append({'label': 'Date Range', 'value': f"{df['date'].min()} to {df['date'].max()}"})
...
def get_map(project_name):
    ...
    if not os.path.exists(map_path) and os.path.exists(csv_path):
        ...
        try:
            ...
            return f"<html><body><h2>Error generating map</h2><pre>{e}</pre></body></html>", 500

    # Check if map exists now
    if not os.path.exists(map_path):
        return f"<html><body><h2>Map not available</h2><p>CSV: {os.path.exists(csv_path)}</p><p>Map path: {map_path}</p></body></html>", 404
...
def ml_map(project_name):
    ...
    if not os.path.exists(ml_dir):
        return "<html><body><h2>ML Analysis Not Available</h2><p>Run ML pipeline first: <code>python run_ml_weekly.py " + project_name + "</code></p></body></html>", 404
    ...
    if not os.path.exists(map_path):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            return f"<html><body><h2>Error Generating ML Map</h2><pre>{e}</pre></body></html>", 500
...
def ml_cluster_detail(project_name, week_id, cluster_label):
    ...
    try:
        ...
        feature_cols = [c for c in cluster_data.columns if c not in [
            'cluster_label', 'outlier_score', 'track_id', 'cluster_status',
            'lat', 'lon', '.geo', 'date', 'satellite', 'spatial_id'
        ]]
