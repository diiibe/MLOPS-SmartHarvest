...
def run_pipeline(roi_coords=config.ROI_TEST, start_date=config.START, end_date=config.END, progress_callback=None):
    ...
    manifest = {
        ...
        "policy": {
            "confirmation": {"coherence_min": config.CONFIRM_COHERENCE_MIN, "persistence_min": config.CONFIRM_PERSISTENCE_MIN}
        },
        "stages": {"extraction": "SUCCESS", "partitioning": "SUCCESS"},  # Implicit
        "artifacts": {
            "pdf_report": {"path": os.path.relpath(export_paths["pdf_report"], start="output"), "type": "pdf"},
            "macro_anomaly_report": {
                "path": os.path.relpath(export_paths["macro_anomaly_report"], start="output"),
                "type": "csv",
            },
            "anomaly_heatmap": {"path": os.path.relpath(export_paths["anomaly_heatmap"], start="output"), "type": "geojson"},
