...
def get_landsat_thermal_data(ROI=config.ROI_TEST, start_date=config.START, end_date=config.END):
    ...
    try:
        ...
        l9 = retrieve_sensor_data("LANDSAT/LC09/C02/T1_L2", roi, start_date, end_date, cloud_max=config.CLOUD_THRESH_LANDSAT)

        l8 = retrieve_sensor_data("LANDSAT/LC08/C02/T1_L2", roi, start_date, end_date, cloud_max=config.CLOUD_THRESH_LANDSAT)
...
def get_sentinel1_data(ROI=config.ROI_TEST, start_date=config.T1_START, end_date=config.T2_END):
    ...
    try:
        ...
        s1_full = retrieve_sensor_data(
            "COPERNICUS/S1_GRD",
            roi,
            start_date,
            end_date,  # Using specific dates
            s1_pol=["VV", "VH"],
            s1_mode="IW",
            s1_orbit="ASCENDING",
        )
...
def get_sentinel2_data(ROI=config.ROI_TEST, start_date=config.T1_START, end_date=config.T2_END):
    ...
    roi = ee.Geometry.Polygon(ROI)
    try:
        s2 = retrieve_sensor_data(
            "COPERNICUS/S2_SR_HARMONIZED", roi, start_date, end_date, cloud_max=config.CLOUD_THRESH  # Using specific dates
        )

        # 2. Auxiliary Collections
        cs_plus = (
            ee.ImageCollection("GOOGLE/CLOUD_SCORE_PLUS/V1/S2_HARMONIZED").filterBounds(roi).filterDate(start_date, end_date)
        )
