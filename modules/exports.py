...
def generate_macro_anomaly_report(run_dir, data=None):
    ...
    df = apply_confirmation_policy(
        df, coherence_min=config.CONFIRM_COHERENCE_MIN, persistence_min=config.CONFIRM_PERSISTENCE_MIN
    )
...
def generate_anomaly_heatmap(run_dir, data=None):
    ...
    features = [
        {
            "type": "Feature",
            "geometry": {"type": "Point", "coordinates": [12.83, 46.10]},
            "properties": {"parcel_id": "p_001", "anomaly_score": 0.1},
        }
    ]
...
def generate_coverage_reliability_report(run_dir, stats=None):
    ...
    if stats:
        ...
        # Dummy
        data = [{"coverage_ratio": 0.95, "valid_pixels": 1000, "reliability_factor": 0.95, "status": "HIGH_CONFIDENCE"}]
