...
class TestGateCConservativePolicy(unittest.TestCase):
    ...
    def test_invariant_check(self, mock_srtm, mock_landsat, mock_st2, mock_st1):
        """
        ...
        if not confirmed_df.empty:
            ...
            if not persistence_violations.empty:
                ...
                self.fail(
                    f"Invariant Violation: {len(persistence_violations)} Confirmed rows have Persistence < {t_persistence}"
                )
        ...
        should_be_confirmed = df[(df["coherence_score"] >= t_coherence) & (df["persistence_score"] >= t_persistence)]
